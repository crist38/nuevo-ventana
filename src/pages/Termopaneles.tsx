...
import type { TermopanelItem } from "../lib/calculos/termopanel"
import { calcularItem, calcularTotal } from "../lib/calculos/termopanel"
...
function CotizadorTermopanelContent() {
  ...
  const handleSaveBudget = async () => {
    ...
    try {
      const user = auth.currentUser;
      if (!user) {
        alert("Debes estar logueado para guardar presupuestos de Termopanel en la Nube.");
        return;
      }
    ...
    <div className="p-4 bg-slate-50 min-h-screen font-sans">
      <header className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4 bg-white p-4 rounded-xl shadow-sm border border-slate-200">
        ...
        <div className="flex gap-2">
          ...
          {user && (
            <button
              onClick={() => navigate(ADMIN_EMAILS.includes(user.email || '') ? '/admin' : '/admin/config')}
...
export const Termopaneles: React.FC = () => {
  return (
